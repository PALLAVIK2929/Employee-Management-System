...
# REST API wrappers for browser UI
@router.get("/tools")
async def list_mcp_tools():
    return [{"name": t.name, "description": t.description, "inputSchema": t.parameters} for t in mcp._tool_manager.list_tools()]

@router.post("/call")
async def call_mcp_tool(request: ToolCallRequest):
    try:
        result = await mcp.call_tool(request.name, request.arguments)
        return {"content": [c.text if hasattr(c, 'text') else str(c) for c in result]}
